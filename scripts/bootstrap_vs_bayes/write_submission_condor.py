...
for (levels, blocksize, lapse_rate, width, nalt, gen_sigmoid, ana_sigmoid) in \
    ((levels, blocksize, lapse_rate, width, nalt, gen_sigmoid, ana_sigmoid)
            for levels     in map(str_no_space, levels)
    ...
    output += "arguments = --nblocks=" + str(nblocks)+\
                         " --fixed-levels=" + levels+\
                         " --blocksize=" + (blocksize)+\
                         " --gen-prm3=" + lapse_rate +\
                         " --gen-prm2=" + width +\
